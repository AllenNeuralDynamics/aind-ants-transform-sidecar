...
class TestDomainValidation:
    ...
    def test_valid_domain_creation(self, valid_bbox: BBox) -> None:
        ...
        domain = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(100, 100, 100))
        assert domain.spacing == (1.0, 1.0, 1.0)
    ...
    def test_domain_without_shape_canonical(self, valid_bbox: BBox) -> None:
        ...
        domain = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox)
        assert domain.spacing == (1.0, 1.0, 1.0)
        assert domain.bbox == valid_bbox
        assert domain.shape_canonical is None
        assert domain.spatial_signature is not None

    def test_domain_rejects_zero_spacing(self, valid_bbox: BBox) -> None:
        """Test that Domain rejects zero spacing."""
        with pytest.raises(ValidationError, match="spacing must be positive and finite"):
            Domain(spacing=(0.0, 1.0, 1.0), bbox=valid_bbox)

    def test_domain_rejects_negative_spacing(self, valid_bbox: BBox) -> None:
        """Test that Domain rejects negative spacing."""
        with pytest.raises(ValidationError, match="spacing must be positive and finite"):
            Domain(spacing=(1.0, -1.0, 1.0), bbox=valid_bbox)

    def test_domain_rejects_infinite_spacing(self, valid_bbox: BBox) -> None:
        """Test that Domain rejects infinite spacing."""
        with pytest.raises(ValidationError, match="spacing must be positive and finite"):
            Domain(spacing=(math.inf, 1.0, 1.0), bbox=valid_bbox)

    def test_domain_rejects_nan_spacing(self, valid_bbox: BBox) -> None:
        """Test that Domain rejects NaN spacing."""
        with pytest.raises(ValidationError, match="spacing must be positive and finite"):
            Domain(spacing=(math.nan, 1.0, 1.0), bbox=valid_bbox)
    ...
    def test_domain_rejects_negative_shape_canonical(self, valid_bbox: BBox) -> None:
        ...
        with pytest.raises(ValidationError, match="shape_canonical must be positive"):
            Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(-100, 100, 100))
    ...
    def test_domain_rejects_zero_shape_canonical(self, valid_bbox: BBox) -> None:
        ...
        with pytest.raises(ValidationError, match="shape_canonical must be positive"):
            Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(100, 0, 100))

    def test_domain_with_anisotropic_spacing(self, valid_bbox: BBox) -> None:
        """Test Domain with different spacing per axis."""
        domain = Domain(spacing=(0.5, 1.0, 2.0), bbox=valid_bbox)
        assert domain.spacing == (0.5, 1.0, 2.0)

    def test_domain_with_very_small_spacing(self, valid_bbox: BBox) -> None:
        """Test Domain with very small spacing values."""
        domain = Domain(spacing=(0.001, 0.001, 0.001), bbox=valid_bbox)
        assert domain.spacing == (0.001, 0.001, 0.001)

    def test_domain_with_large_spacing(self, valid_bbox: BBox) -> None:
        """Test Domain with large spacing values."""
        domain = Domain(spacing=(100.0, 100.0, 100.0), bbox=valid_bbox)
        assert domain.spacing == (100.0, 100.0, 100.0)
...
class TestSpatialHashing:
    ...
    def test_spatial_signature_reproducible(self, valid_bbox: BBox) -> None:
        ...
        domain1 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(100, 100, 100))
        domain2 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(100, 100, 100))
        assert domain1.spatial_signature == domain2.spatial_signature

    def test_spatial_signature_changes_with_spacing(self, valid_bbox: BBox) -> None:
        """Test that spatial signature changes when spacing changes."""
        domain1 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox)
        domain2 = Domain(spacing=(1.1, 1.0, 1.0), bbox=valid_bbox)
        assert domain1.spatial_signature != domain2.spatial_signature
    ...
    def test_spatial_signature_changes_with_bbox(self) -> None:
        ...
        domain1 = Domain(spacing=(1.0, 1.0, 1.0), bbox=bbox1)
        domain2 = Domain(spacing=(1.0, 1.0, 1.0), bbox=bbox2)
        assert domain1.spatial_signature != domain2.spatial_signature
    ...
    def test_spatial_signature_changes_with_shape(self, valid_bbox: BBox) -> None:
        ...
        domain1 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(100, 100, 100))
        domain2 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(101, 100, 100))
        assert domain1.spatial_signature != domain2.spatial_signature
    ...
    def test_spatial_signature_differs_with_without_shape(self, valid_bbox: BBox) -> None:
        ...
        domain1 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=(100, 100, 100))
        domain2 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, shape_canonical=None)
        assert domain1.spatial_signature != domain2.spatial_signature
    ...
    def test_spatial_signature_validation_accepts_correct(self, valid_bbox: BBox) -> None:
        ...
        domain2 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, spatial_signature=sig)
    ...
    def test_spatial_signature_validation_rejects_mismatch(self, valid_bbox: BBox) -> None:
        ...
        with pytest.raises(ValidationError, match="spatial_signature mismatch"):
            Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox, spatial_signature=wrong_sig)
    ...
    def test_spatial_hash_sensitivity_to_small_changes(self, valid_bbox: BBox) -> None:
        ...
        # Change spacing by a tiny amount (well within hash precision of 9 decimal places)
        domain1 = Domain(spacing=(1.0, 1.0, 1.0), bbox=valid_bbox)
        domain2 = Domain(spacing=(1.0000000001, 1.0, 1.0), bbox=valid_bbox)
        # Should be same (within 9 decimal places)
        assert domain1.spatial_signature == domain2.spatial_signature

        # Change spacing beyond hash precision (10^-9)
        domain3 = Domain(spacing=(1.00000001, 1.0, 1.0), bbox=valid_bbox)
    ...
    def test_spatial_hash_with_extreme_values(self) -> None:
        ...
        domain = Domain(spacing=(100.0, 100.0, 100.0), bbox=bbox)
        assert domain.spatial_signature is not None
        assert domain.spatial_signature.blake2b.startswith("b2:")
